...
class Game:
    ...
    def __init__(self, width, height, max_time, bomb_time, expl_duration, expl_range, sprite_dir="../../bomberman/sprites/"):
        self.world = RealWorld.from_params(
            width, height, max_time, bomb_time, expl_duration, expl_range)
    ...
    def fromfile(cls, fname, sprite_dir="../../bomberman/sprites/"):
        with open(fname, 'r') as fd:
            ...
            while row and row[0] == '|':
                height = height + 1
                if len(row) != width + 3:
                    raise RuntimeError("Row", height, "is not",
                                       width, "characters long")
                row = fd.readline()
            # Create empty world
            gm = cls(width, height, max_time, bomb_time,
                     expl_duration, expl_range, sprite_dir)
                ...
                for x in range(0, width):
                    if ln[x+1] == 'E':
                        if not gm.world.exitcell:
                            ...
                            raise RuntimeError(
                                "There can be only one exit cell, first one found at", x, y)
                    elif ln[x+1] == 'W':
    ...
    def load_gui(self, board_width, board_height):
        ...
        self.bomberman_sprite = pygame.image.load(
            self.sprite_dir + "bomberman.png")
        self.bomberman_sprite = pygame.transform.scale(
            self.bomberman_sprite, rect)
        self.monster_sprite = pygame.image.load(
            self.sprite_dir + "monster.png")
        ...
        self.explosion_sprite = pygame.image.load(
            self.sprite_dir + "explosion.png")
        self.explosion_sprite = pygame.transform.scale(
            self.explosion_sprite, rect)
    ...
    def display_gui(self):
        for x in range(self.world.width()):
            for y in range(self.world.height()):
                ...
                pygame.draw.rect(self.screen, (65, 132, 15), [
                                 left, top, self.block_width, self.block_height])
